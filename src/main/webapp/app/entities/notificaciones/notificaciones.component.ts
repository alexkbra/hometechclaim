...
export class NotificacionesComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.notificacionesService
      ...
      .subscribe(
        (res: HttpResponse<INotificaciones[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: INotificaciones): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInNotificaciones(): void {
    this.eventSubscriber = this.eventManager.subscribe('notificacionesListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: INotificaciones[] | null, headers: HttpHeaders, page: number): void {
