...
export class NovedadUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    ...
    subtitulo: [null, [Validators.required, Validators.minLength(5), Validators.maxLength(250)]],
  ...
  updateForm(novedad: INovedad): void {
    this.editForm.patchValue({
      ...
      imagenContentType: novedad.imagenContentType
    });
  }
  ...
  save(): void {
    this.isSaving = true;
    const novedad = this.createFromForm();
    if (novedad.id !== undefined) {
  ...
  private createFromForm(): INovedad {
    return {
      ...new Novedad(),
